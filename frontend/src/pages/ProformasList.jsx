...
const ProformasList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const proformasRes = await axios.get("http://localhost:8000/api/proformas/");

        const proformas = proformasRes.data.map((item) => ({
          id: item.id || item._id,
          type: "Proforma",
          code: item.proforma_number || item.numero || "SIN-CÓDIGO",
          date: item.date || item.fecha || "Sin fecha",
          status: item.status || "Por hacer",
          creado_por: item.created_by || "Desconocido",
        }));

        const sorted = proformas
          .map((item) => ({
            ...item,
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      await axios.patch(
        `http://localhost:8000/api/proformas/${id}/`,
        { status: newStatus }
      );
      setAllDocs((prev) =>
        prev.map((doc) =>
  ...
  const handleDownload = async (doc) => {
    try {
      const url = `http://localhost:8000/api/proformas/${doc.id}/pdf/`;
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="main-content">
        ...
        <h2 className="section-title">Proformas Recientes</h2>
